      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid items-center justify-center grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {pip.map((p) => (
            <div
              key={p.id}
              className="group flex flex-col items-center md:items-start gap-4 border-t border-white/10 pt-6 hover:border-orange-500 transition-colors duration-300"
            >
              {/* Icon */}
              <img
                src={p.img}
              ...
              {/* Title */}
              <h3 className="text-white text-sm font-black uppercase tracking-widest text-center md:text-left group-hover:text-orange-500 transition-colors duration-300">
                {p.title}
              </h3>

              {/* Description */}
              <p className="text-gray-500 text-sm leading-relaxed text-center md:text-left group-hover:text-gray-400 transition-colors duration-300">
                {p.desc}
              </p>
            </div>
          ))}
